# decentral_agent_rescheduling_Task10.py
...
class DecentralAgent(Agent):
    def __init__(
        ...
        self._hb_periodic_handle = None

        # -------------------------------
        # Countermeasure storage
        # -------------------------------
        # last schedule ever received from each agent (for freeze)
        self.last_known_schedule_by_aid: dict[str, list[float]] = {}
        # contributions we keep fixed for dead agents (freeze mode)
        self.dead_fixed_contrib: dict[str, list[float]] = {}
        self.FAILED_DEVICE_MODE = "freeze"  # "freeze" or "zero"

        # -------------------------------
        # ✅ keep only ONE optimization loop running
        # -------------------------------
        self._opt_task = None
    ...
    def _stop_optimization_loop(self):
        """Cancel optimization loop if running."""
    ...
    def _stop_heartbeat(self):
        if self._hb_periodic_handle is None:
            return
        try:
            self._hb_periodic_handle.cancel()
        except Exception:
            pass
    ...
    async def _mark_agent_down(self, down_aid: str, reported_by: str, ts: float):
        ...
        # remove from memory so its negotiation is ignored
        ...
        # -------------------------------
        # Countermeasure: FREEZE last known schedule of dead agent
        # -------------------------------
        if self.FAILED_DEVICE_MODE == "freeze":
            last_sched = self.last_known_schedule_by_aid.get(down_aid, None)
            if last_sched is not None:
                self.dead_fixed_contrib[down_aid] = list(last_sched)

        logging.warning(
            f"[Task10] {self.aid} detected {down_aid} as DOWN. Gossiping... "
            f"(freeze={down_aid in self.dead_fixed_contrib})"
        )
        ...
        if isinstance(content, AgentDownMsg) and is_neighbor:
            if content.down_aid not in self.dead_aids and content.down_aid != self.aid:
                self.schedule_instant_task(
                    self._mark_agent_down(content.down_aid, content.reported_by, content.ts)
                )
            return
    ...
    async def handle_any_message(self, content, meta):
        ...
        if isinstance(content, FlexMsg):
            ...
            # cache for freeze
            self.last_known_schedule_by_aid[content.aid] = list(content.schedule)
    ...
    async def optimization_loop(self):
        ...
        try:
            self.publish()
            while not self.done.done() and not self.failed:
                await self.perceive()
                self.update()
                self.publish()
    ...
    def update(self):
        ...
        # frozen contributions from dead agents (freeze mode)
        if self.FAILED_DEVICE_MODE == "freeze":
            for aid, sched in self.dead_fixed_contrib.items():

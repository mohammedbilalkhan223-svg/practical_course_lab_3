...
class DecentralAgent(Agent):
    ...
    async def handle_any_message(self, content, meta):
        if not hasattr(self, "_drop_rng"):
            self._drop_rng = random.Random(123 + int(self.aid.split("_")[1]))
            self._received = 0
            self._dropped = 0
        sender = sender_addr(meta)
        ...
        if is_neighbor:
            ...
            self._received += 1
            drop_now = (w > 0 and self._drop_rng.random() < w)
            if drop_now:
                self._dropped += 1
            if self._received % 100 == 0:
                print(f"{self.aid}: dropped {self._dropped}/{self._received} incoming msgs")
